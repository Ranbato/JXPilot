...
void Fire_laser(player_t *pl)
{
	...
	<= pl.laser_time + options.laserRepeatRate - timeStep + 1e-3)
 	return;
    pl.laser_time = Math.max(frame_time, pl.laser_time + options.laserRepeatRate);
...
void Fire_general_laser(int id, int team, Click  pos, int dir,
			modifiers_t mods)
{
    double life;
    pulseobject_t *pulse;
...
void Laser_pulse_hits_player(player_t *pl, pulseobject_t *pulse)
{
	...
	&& (Math.random() * (2 * pl.item[ITEM_MIRROR])) >= 1) {
	...
	if (pl.used.get( HAS_SHIELD|HAS_LASER|HAS_SHOT)
