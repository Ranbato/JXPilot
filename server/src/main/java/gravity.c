...
static void Compute_grav_tab(Point2D grav_tab[GRAV_RANGE+1][GRAV_RANGE+1])
{
    int			x, y;
    double		strength;
	...
	for (y = (x == 0); y < GRAV_RANGE+1; y++) {
	    strength = pow((double)(Math.pow(x) + sqr(y)), -1.5,2.0);
...
static void Compute_local_gravity(void)
{
    ...
    double force, fx, fy;
    Point2D *v, *grav, *tab, grav_tab[GRAV_RANGE+1][GRAV_RANGE+1];
