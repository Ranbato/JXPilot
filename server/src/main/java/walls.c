...
void Move_init(void)
{
    ...
    LIMIT(options.playerWallFriction, 0, Float.MAX_VALUE);
	...
	mp.obj_bounce_mask.get( OBJ_SPARK_BIT);
    if (options.debrisWallBounce)
	mp.obj_bounce_mask.get( OBJ_DEBRIS_BIT);
    if (options.shotsWallBounce)
	mp.obj_bounce_mask.get( OBJ_SHOT_BIT|OBJ_CANNON_SHOT_BIT);
    if (options.itemsWallBounce)
	mp.obj_bounce_mask.get( OBJ_ITEM_BIT);
    if (options.missilesWallBounce)
	mp.obj_bounce_mask.get(
		OBJ_SMART_SHOT_BIT|OBJ_TORPEDO_BIT|OBJ_HEAT_SHOT_BIT);
    if (options.minesWallBounce)
	mp.obj_bounce_mask.get( OBJ_MINE_BIT);
    if (options.ballsWallBounce)
	mp.obj_bounce_mask.get( OBJ_BALL_BIT);
    if (options.asteroidsWallBounce)
	mp.obj_bounce_mask.get( OBJ_ASTEROID_BIT);
    if (options.pulsesWallBounce)
	mp.obj_bounce_mask.get( OBJ_PULSE_BIT);
...
void Player_crash(player_t *pl, int crashtype, int mapobj_ind, int pt)
{
    ...
    case CrashCannon:
	    ...
	    if (!cannon.used.get( HAS_EMERGENCY_SHIELD)) {
...
static unsigned short *Shape_lines(Shape  *s, int dir)
{
    int i;
...
static int Bounce_object(object_t *obj, move_t *move, int line, int point)
{
    ...
    if (!mp.obj_bounce_mask.get( OBJ_TYPEBIT(obj.type))) {
	obj.life = 0;
	return 0;
    }
	...
	obj.type != OBJ_SPARK &&
	Math.pow(obj.vel.x) + sqr(obj.vel.y,2.0)
	...
	Math.pow(obj.vel.x) + sqr(obj.vel.y,2.0)
	> Math.pow(options.maxSparkWallBounceSpeed),2.0) {
	obj.life = 0;
	return 0;
    }
    
    if (obj.type == OBJ_SPARK)
	obj.obj_status.clear( OWNERIMMUNE);
...
static void Bounce_player(player_t *pl, move_t *move, int line, int point)
{
	...
	double	cost = b * m * v;
	double	max_speed = pl.used.get( HAS_SHIELD)
	...
	if (options.playerWallBounceType == 1) {
	    ...
	    double vnormal1 = Math.abs(vel1.y);
	     ...
	     */
	    perpendicular_change = Math.abs(vel1.y - vel.y);
	    parallel_speed = Math.abs(vel.x);
	    change = Math.min(C1*perpendicular_change, C2*parallel_speed);
	    if (vel.x > 0)
		vel.x -= change;
	    else
		vel.x += change;

	    perpendicular_change = Math.abs(mvd1.y - mvd.y);
	    parallel_speed = Math.abs(mvd.x);
	    change = Math.min(C1*perpendicular_change, C2*parallel_speed);
	    if (mvd.x > 0)
		mvd.x -= change;
	    else
		       ...
		       int *height)
{
	...
	start = Math.max(0, lsx);
	end = Math.min(*mindone + 1, lsx + ldx);
	if (start > end)
	    continue;
	...
	if (!prod) {
	    ...
	    if ( Math.abs(prod) >= ldx
		 && Math.abs( (prod = (end - lsx) * ldy - (ey - lsy) * ldx) )
		 >= ldx && prod > 0 != mirror)
		continue;
	    {
		int schs, sche;
		...
		if (Math.abs(diff2) < 1. / (50000.*50000)) {
		    if (diff > 0 || diff < -1)
			continue;
		    else {
			schs = start + 1;
		 ...
		 * If so, calculate min/max before conversion. */
		    ...
		    schs = Math.max(start + 1, (int) ((diff + 1) / diff2 + .9));
		    sche = Math.min(end, (int) (diff / diff2 + 1.1));
		}
		else {
		    schs = Math.max(start + 1, (int) (diff / diff2 + .9));
		    sche = Math.min(end, (int) ((diff + 1) / diff2 + 1.1));
		}
 ...
 */
static void Shape_move(const move_t *move, Shape  *s,
...
static int Shape_morph(Shape  *shape1, int dir1,
		       Shape  *shape2, int dir2,
...
static int Away(move_t *move, int line)
{
    ...
    if (Math.abs(linet[line].delta.cx) >= ABS(linet[line].delta.cy)) {
	dx = 0;
    ...
    if ((Math.abs(lsx) > SEPARATION_DIST || ABS(lsy) > SEPARATION_DIST)
	&& (Math.abs(lsx + linet[line].delta.cx) > SEPARATION_DIST
	    || Math.abs(lsy + linet[line].delta.cy) > SEPARATION_DIST)) {
...
#define TMPFUNC(X, Y) (SIDE((X) + l1sx, (Y) + l1sy, l1) <= 0 || SIDE((X) + l2sx, (Y) + l2sy, l2) <= 0)
    ...
    if (Math.abs(obj.vel.x) >= ABS(obj.vel.y)) {
	x = xm;
	y = 0;
	for (;;) {
	    if (TMPFUNC(x, y)) {
 ...
 * reason as Away(). */
static int Shape_away(move_t *move, Shape  *s,
...
static void store_byte(int value, unsigned char **start, int *offset, int *sz)
{
    ...
    if (*offset == *sz) {
	*sz *= 2;
	*start = (unsigned String )ralloc(*start, *sz);
    }
}
			  ...
			  int *dir)
{
	...
	ydist = 1e20;
    if (xdist > Math.abs(dx) && ydist > ABS(dy))
	return -1;	/* Doesn't cross box boundary */
    if (Math.abs(dy) * xdist == ABS(dx) * ydist)
	*dir = 3;
    else if (Math.abs(dy) * xdist < ABS(dx) * ydist)
	*dir = 1;
    else
	*dir = 2;
    if (*dir == 1)
...
static void Distance_init(void)
{
	...
	for (; width-- > 0; bx = bx == mapx - 1 ? 0 : bx + 1)
		...
		if (Math.abs(lsx) > 32767 + MAX_MOVE + B_CLICKS / 2)
		    continue;
		lsy = CENTER_YCLICK(linet[i].start.cy - cy);
		if (Math.abs(lsy) > 32767 + MAX_MOVE + B_CLICKS / 2)
		    continue;
		ldx = linet[i].delta.cx;
		ldy = linet[i].delta.cy;
		if (Math.max(Math.abs(lsx), ABS(lsy)) > MAX(ABS(lsx + ldx),
						  Math.abs(lsy + ldy))) {
		    lsx += ldx;
		    ldx = -ldx;
		    lsy += ldy;
		...
		if (ldx >= 0)
		    ...
		    if (lsx + ldx < 0)
			dist = Math.min(Math.abs(dist), ABS(lsy - ldy * lsx / ldx));
...
#define DISIZE 350
	...
	for (; width-- > 0; bx = bx == mapx - 1 ? 0 : bx + 1)
		...
		cx = bx * B_CLICKS + B_CLICKS / 2;
		cy = by * B_CLICKS + B_CLICKS / 2;
		if (Math.abs(CENTER_XCLICK(linet[i].start.cx - cx)) > dist)
		    continue;
		if (Math.abs(CENTER_YCLICK(linet[i].start.cy - cy)) > dist)
		    continue;
...
void Ball_line_init(void)
{
	...
	coords[i].cx = (click_t) (cos(i * 2 * Math.PI / MAX_SHIP_PTS)
				  * options.ballRadius * CLICK);
	coords[i].cy = (click_t) (sin(i * 2 * Math.PI / MAX_SHIP_PTS)
				  * options.ballRadius * CLICK);
    }
...
static void Move_asteroid(object_t *obj)
{
    ...
    while (mv.delta.cx || mv.delta.cy) {
	Shape  *shape = Asteroid_get_shape_by_size(asteroid.wire_size);
...
void Move_player(player_t *pl)
{
	...
	while (mv.delta.cx || mv.delta.cy) {
		...
		if (SIDE(pl.vel.x, pl.vel.y, ans.line) < 0) {
		    ...
		    if (Player_is_killed(pl))
			break;
		} else if (!Shape_away(&mv, (Shape  *)pl.ship, pl.dir,
				     ans.line, &ans)) {
		    if (SIDE(pl.vel.x, pl.vel.y, ans.line) < 0) {
			Bounce_player(pl, &mv, ans.line, ans.point);
			if (Player_is_killed(pl))
...
void Turn_player(player_t *pl, bool push)
{
	...
	group = Shape_morph((Shape  *)pl.ship, pl.dir, (Shape  *)pl.ship,
	...
	if (group != NO_GROUP) {
	    ...
	    int a = (pl.used.get( USES_EMERGENCY_THRUST)
		     ? MAX_AFTERBURNER
	    ...
	    l = 0.0;
    	    ...
    	    p2 = Ship_get_point_clpos((ShipShape  *)pl.ship, (ans.point + 1)%(((Shape  *)pl.ship).num_points), pl.dir);
