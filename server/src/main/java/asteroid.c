 ...
 * Prototypes.
 */
static void Make_asteroid(Click  pos, int size, int dir, double speed);
...
void Break_asteroid(wireobject_t *asteroid)
{
	...
	dir1 = MOD2((int)(dir
			  ...
			  - (Math.random() * ASTEROID_DELTA_DIR / 4)), RES);
	dir2 = MOD2((int)(dir
			  + ASTEROID_DELTA_DIR / 4
			  + (Math.random() * ASTEROID_DELTA_DIR / 4)), RES);
	speed1 = (speed * (1 - ASTEROID_DUST_FACT))
		 / tcos(Math.abs((int)dir - dir1));
	speed2 = (speed * (1 - ASTEROID_DUST_FACT))
		 / tcos(Math.abs(dir2 - (int)dir));
	...
	&& (Math.random() < options.asteroidItemProb)) {
	int nitems = (int)(Math.random() * options.asteroidMaxItems) + 1;
	Point2D vel;
	...
	for (i = 0; i < nitems; i++) {
	    ...
	    item_dir = (int)(Math.random() * RES);
	    item_speed = Math.random() * 10;
...
static void Make_asteroid(Click  pos, int size, int dir, double speed)
{
    ...
    asteroid.wire_turnspeed = 0.02 + Math.random() * 0.05;
    asteroid.wire_rotation = (int)(Math.random() * RES);
    asteroid.wire_size = size;
    asteroid.wire_type = (uint8_t)(Math.random() * 256);
...
static void Place_asteroid(void)
{
	...
	if (con) {
	    dir = (int)(Math.random() * RES);
	    dist = (int)(Math.random() * ((options.asteroidConcentratorRadius
				     * BLOCK_CLICKS) + 1));
    ...
    if (okay)
	Make_asteroid(pos,
		      (int)(1 + Math.random() * ASTEROID_MAX_SIZE),
		      (int)(Math.random() * RES),
...
void Asteroid_update(void)
{
	...
	if (world.asteroids.num + incr < world.asteroids.max) {
	    if ((Math.random() * world.asteroids.chance) < 1.0)
		Place_asteroid();
	}
    }
}
...
Shape  asteroid_wire1;
Shape  asteroid_wire2;
...
void Asteroid_line_init(void)
{
    ...
    static Click  coords2[MAX_SHIP_PTS];
    static Click  coords3[MAX_SHIP_PTS];
    static Click  coords4[MAX_SHIP_PTS];

    asteroid_wire1.num_points = MAX_SHIP_PTS;
    for (i = 0; i < MAX_SHIP_PTS; i++) {
	asteroid_wire1.pts[i] = coords1 + i;
	coords1[i].cx
	    = (click_t)(cos(i * 2 * Math.PI / MAX_SHIP_PTS) * ASTEROID_RADIUS(1));
	coords1[i].cy
	    = (click_t)(sin(i * 2 * Math.PI / MAX_SHIP_PTS) * ASTEROID_RADIUS(1));
    }
	...
	coords2[i].cx
	    = (click_t)(cos(i * 2 * Math.PI / MAX_SHIP_PTS) * ASTEROID_RADIUS(2));
	coords2[i].cy
	    = (click_t)(sin(i * 2 * Math.PI / MAX_SHIP_PTS) * ASTEROID_RADIUS(2));
    }
	...
	coords3[i].cx
	    = (click_t)(cos(i * 2 * Math.PI / MAX_SHIP_PTS) * ASTEROID_RADIUS(3));
	coords3[i].cy
	    = (click_t)(sin(i * 2 * Math.PI / MAX_SHIP_PTS) * ASTEROID_RADIUS(3));
    }
	...
	coords4[i].cx
	    = (click_t)(cos(i * 2 * Math.PI / MAX_SHIP_PTS) * ASTEROID_RADIUS(4));
	coords4[i].cy
	    = (click_t)(sin(i * 2 * Math.PI / MAX_SHIP_PTS) * ASTEROID_RADIUS(4));
    }
