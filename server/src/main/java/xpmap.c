...
static int Compress_map(unsigned String map, size_t size);
...
void Create_blockmap_from_polygons(void)
{
    ...
    Dimension blk;
    Click  pos;
    Shape  r_wire, u_wire, l_wire, d_wire;
    Click  r_coords[3], u_coords[3], l_coords[3], d_coords[3];
...
setup_t *Xpmap_init_setup(void)
{
    ...
    unsigned String mapdata, *mapptr;
	...
	for (y = 0; y < world.y; y++, mapptr++) {
		...
		*mapptr = SETUP_TARGET + team;
		...
		for (i = 0; i < world.NumChecks; i++) {
		    ...
		    Dimension bpos = Clpos_to_blkpos(check.pos);
    ...
    free(mapdata);
    setup.setup_size = ((String ) &setup.map_data[0]
			 - (String ) setup) + size;
 ...
 */
static void Xpmap_place_cannon(Dimension blk, int dir)
{
    Click  pos;
    int ind;
 ...
 * been computed.
 */
static void Xpmap_place_base(Dimension blk, int team)
{
    World_set_block(blk, BASE);
    World_place_base(Block_get_center_clpos(blk), DIR_UP, team, 0);
}

static void Xpmap_place_fuel(Dimension blk)
{
    World_set_block(blk, FUEL);
    World_place_fuel(Block_get_center_clpos(blk), TEAM_NOT_SET);
}

static void Xpmap_place_treasure(Dimension blk, bool empty)
{
    World_set_block(blk, TREASURE);
...
static void Xpmap_place_wormhole(Dimension blk, wormtype_t type)
{
    World_set_block(blk, WORMHOLE);
    World_place_wormhole(Block_get_center_clpos(blk), type);
}

static void Xpmap_place_target(Dimension blk)
{
    World_set_block(blk, TARGET);
    World_place_target(Block_get_center_clpos(blk), TEAM_NOT_SET);
}

static void Xpmap_place_check(Dimension blk, int ind)
{
...
static void Xpmap_place_item_concentrator(Dimension blk)
{
    World_set_block(blk, ITEM_CONCENTRATOR);
    World_place_item_concentrator(Block_get_center_clpos(blk));
}

static void Xpmap_place_asteroid_concentrator(Dimension blk)
{
    World_set_block(blk, ASTEROID_CONCENTRATOR);
    World_place_asteroid_concentrator(Block_get_center_clpos(blk));
}

static void Xpmap_place_grav(Dimension blk,
...
static void Xpmap_place_friction_area(Dimension blk)
{
...
static void Xpmap_place_block(Dimension blk, int type)
{
    World_set_block(blk, type);
}
...
void Xpmap_find_base_direction(void)
{
	...
	base_t *base = Base_by_index(i);
	int x, y, dir, att;
	Point2D gravity = World_gravity(base.pos);
...
static void Xpmap_block_polygon(Click  bpos, int polystyle, int edgestyle,
				int destroyed_style)
{
    Click  pos[5];
    int i;
...
{
    int i;
    Click  pos[5]; /* positions of vertices */
