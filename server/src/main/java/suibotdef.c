...
static void Robot_suibot_message(player_t *pl, String str);
 ...
 */
static void Robot_suibot_message(player_t *pl, String message)
{
...
static bool Get_object_proximity(player_t *pl, object_t *shot, double sqmaxdist,int maxtime, object_proximity_t *object_proximity){
  ...
  time_until_closest =
    ...
    ((Math.pow(delta_velx) + sqr(delta_vely)),2.0);
  ...
  sqdistance =
    (Math.pow(delta_velx) + sqr(delta_vely)) * sqr(time_until_closest,2.0)  +
    2 * (delta_velx * delta_x + delta_vely * delta_y) * time_until_closest +
    Math.pow(delta_x) + sqr(delta_y,2.0);
...
/*  change to use   struct dangerous_shot_data *shotsarray; */
  ...
  //  player_t *opponent;
    ...
    if (NumObjs >= options.cellGetObjectsThreshold)
        Cell_get_objects(pl.pos, Math.max(hori_blocks, vert_blocks),
	...
	if (!shot.type.get( killing_shots ))
	    continue;
	...
	sqdistance = 
	    (Math.pow(delta_velx) + sqr(delta_vely)) * sqr(time_shot_closest,2.0)  
	    + 2 * (delta_velx * delta_x + delta_vely * delta_y) 
	    * time_shot_closest 
	    + Math.pow(delta_x) + sqr(delta_y,2.0);
	
#define SQ_SHIP_SZ_11 Math.pow(1.1 * PIXEL_TO_CLICK(SHIP_SZ),2.0)
#define SQ_SHIP_SZ_25 Math.pow(2.5 * PIXEL_TO_CLICK(SHIP_SZ),2.0)
...
/*    double direction_pl,direction_evade1,direction_evade2;
    double norm_vel= sqrt(Math.pow(delta_velx)+sqr(delta_vely),2.0);
    double norm_xy = sqrt(Math.pow(delta_x)+sqr(delta_y),2.0);
...
void Robot_move_randomly(player_t *pl){
  ...
  /* Move randomly */
  if(Math.random()<0.25)
    pl.turnvel = ((Math.random()*RES)- RES / 2)*0.3;
...
#define DD false /* debug */
    ...
    sqr_a   = Math.pow(pl.power / pl.mass * CLICK,2.0);       /* acceleration */
    sqr_b   = Math.pow(velx)+sqr(vely,2.0);  /* velocity     */
    sqr_c   = Math.pow(x)+sqr(y,2.0);        /* distance     */
...
/*       double tmp_length= */
/* 	abs(LENGTH(tmp_x,tmp_y)- 0.5 * (CLICK * pl.power / pl.mass) * Math.pow(time),2.0); */
/*       if(tmp_length < 3000) */
...
/*     } */
	...
	function = 
	    -0.25 * sqr_a * Math.pow(sqr(time),2.0) 
	    + sqr_b * Math.pow(time,2.0)
	    + 2 * b_dot_c * time 
	    + sqr_c;
	deviation=
	    -sqr_a * Math.pow(time,2.0) * time 
	    + 2 * sqr_b * time 
	    + 2 * b_dot_c;
...
//	if( abs(delta_time)< 0.001* abs(time))
      if(abs(function) < 10)
//	if(abs(LENGTH(tmp_x,tmp_y)- 0.5 * (CLICK * pl.power/pl.mass) * Math.pow(time)) < 10 ,2.0)
	    break;
    }
...
*/    
	...
	printf("            difference: %.3f\n",LENGTH(x_tgo,y_tgo)- 0.5 * sqrt(sqr_a) * Math.pow(time),2.0);
     ...
     */    
//    if(sqr_c > Math.pow(x+x_tgo)+sqr(y+y_tgo),2.0){
//	x_tgo=x;
//	y_tgo=y;
//	}
...
void Robot_attack_player(player_t *pl, player_t *opponent){/*attack_player*/   
    ...
    tmp_b = Math.pow(velx) + sqr(vely) - sqr(options.shotSpeed * CLICK,2.0);
    
    tmp_c = Math.pow(tmp_a) - tmp_b * (sqr(dcx) + sqr(dcy),2.0);
	...
	if(t<0){
	    ...
	    if(Math.random() >0.5){ Fire_normal_shots(pl);}
	    return;
	}
...
#define D2 false
	...
	dcx = dcx + opponent.acc.x * t * Math.pow(sqr(Math.random()),2.0);
	dcy = dcy + opponent.acc.y * t * Math.pow(sqr(Math.random()),2.0);
	
	/* actually, it should be acc * Math.pow(time,2.0); 
    ...
    if(Math.random() > 0.8) /* spread shots */
...
static void Robot_suibot_play(player_t *pl)
{
  ...
  if(ship_dist_closest <  maxdist && closest_opponent.used.get( HAS_SHIELD)){
