...
void Cannon_update(bool tick)
{
	...
	if (tick) {
	    ...
	    if (!c.used.get( HAS_EMERGENCY_SHIELD)
		&& !c.used.get( USES_PHASING_DEVICE)
		&& (c.damaged <= 0)
		&& (c.tractor_count <= 0)
		&& Math.random() * 16 < 1)
		Cannon_check_fire(c);
...
void Cannon_throw_items(cannon_t *c)
{
	...
	while (c.item[i] > 0) {
	    int amount = world.items[i].max_per_pack
			 - (int)(Math.random() * (1 + world.items[i].max_per_pack
...
static int Cannon_in_danger(cannon_t *c)
{
	...
	if (shot.life <= 0)
	    continue;
	if (!OBJ_TYPEBIT(shot.type).get( kill_shots))
	    continue;
...
static void Cannon_defend(cannon_t *c, int defense)
{
	...
	c.used.get( HAS_EMERGENCY_SHIELD);
...
static int Cannon_select_weapon(cannon_t *c)
{
	...
	&& (int)(Math.random() * (c.item[ITEM_LASER] + 1)))
	return CW_LASER;
    if (c.item[ITEM_ECM]
	&& Math.random() < 0.333)
	return CW_ECM;
	...
	&& c.item[ITEM_FUEL]
	&& (int)(Math.random() * ((c.item[ITEM_EMERGENCY_THRUST] ?
		      MAX_AFTERBURNER :
...
static void Cannon_aim(cannon_t *c, int weapon, player_t **pl_p, int *dir)
{
	...
	if (smartness > 2
	    || (int)(Math.random() * Math.pow(smartness + 1)),2.0)
	...
	if (Player_is_paused(pl)
		...
		&& (int)(Math.random() * (pl.item[ITEM_CLOAK] + 1))
		   > (int)(Math.random() * (c.item[ITEM_SENSOR] + 1)))
	    || (smartness > 2
	...
	*dir = c.dir;
    ...
    case 1:
	*dir = c.dir;
	*dir += (int)((Math.random() - 0.5f) * CANNON_SPREAD);
	break;
    case 2:
...
static void Cannon_fire(cannon_t *c, int weapon, player_t *pl, int dir)
{
	...
	if (Math.random() < 0.5) {	/* place mine in front of cannon */
	    ...
	    Mods_set(&mods, ModsMini,
		     (int)(Math.random() * MODS_MINI_MAX) + 1);
	...
	/* Because cannons don't have missile racks, all mini missiles
	   ...
	   mods.mini = (int)(Math.random() * MODS_MINI_MAX) + 1;
	   mods.spread = (int)(Math.random() * (MODS_SPREAD_MAX + 1));
	*/

	/* smarter cannons use more advanced missile types */
	switch ((int)(Math.random() * (1 + smartness))) {
	default:
	...
	c.tractor_is_pressor = (Math.random() * (smartness + 1) >= 1);
	c.tractor_target_id = pl.id;
	c.tractor_count = 11 + Math.random() * (3 * smartness + 1);
	break;
    case CW_TRANSPORTER:
	...
	if ((Math.random() * (c.item[ITEM_EMERGENCY_THRUST] + 1)) >= 1) {
	    Make_debris(c.pos,
			zero_vel,
			NO_ID,
			c.team,
...
hitmask_t Cannon_hitmask(cannon_t *cannon)
{
	...
	return ALL_BITS;
    if (world.rules.mode.get( TEAM_PLAY) && options.teamImmunity)
...
{
    Dimension blk = Clpos_to_blkpos(cannon.pos);
    int i;
...
bool Cannon_hitfunc(group_t *gp, const move_t *move)
{
    ...
    if (!cannon_mask.get( OBJ_TYPEBIT(obj.type)))
	return false;
