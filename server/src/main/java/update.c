...
{
    if (obj.obj_status.get( GRAVITY)) {
	Point2D gravity = World_gravity(obj.pos);
...
static void Transport_to_home(player_t *pl)
{
    ...
    if (world.rules.mode.get( TIMING) && pl.round) {
	int check;
...
void Phasing(player_t *pl, bool on)
{
	...
	pl.used.get( USES_PHASING_DEVICE);
	...
	pl.used.clear( USES_PHASING_DEVICE);
	if (pl.phasing_left <= 0) {
	    if (pl.item[ITEM_PHASING] <= 0)
		pl.have.clear( HAS_PHASING_DEVICE);
	}
	pl.obj_status.get( GRAVITY);
...
void Deflector(player_t *pl, bool on)
{
	...
	if (!pl.used.get( USES_DEFLECTOR)
	    ...
	    pl.used.get( USES_DEFLECTOR);
	    sound_play_player(pl, DEFLECTOR_SOUND);
	}
    } else {
	if (pl.used.get( USES_DEFLECTOR)) {
	    pl.used.clear( USES_DEFLECTOR);
	    sound_play_player(pl, DEFLECTOR_SOUND);
	}
...
void Emergency_thrust(player_t *pl, bool on)
{
	...
	if (!Player_uses_emergency_thrust(pl)) {
	    pl.used.get( USES_EMERGENCY_THRUST);
	    sound_play_sensors(pl.pos, EMERGENCY_THRUST_ON_SOUND);
	}
    } else {
	if (Player_uses_emergency_thrust(pl)) {
	    pl.used.clear( USES_EMERGENCY_THRUST);
...
void Emergency_shield(player_t *pl, bool on)
{
	...
	if (pl.have.get( HAS_EMERGENCY_SHIELD)) {
	    ...
	    if (!pl.used.get( USES_EMERGENCY_SHIELD)) {
		pl.used.get( USES_EMERGENCY_SHIELD);
	...
	if (!DEF_HAVE.get( HAS_SHIELD)) {
	    ...
	    pl.used.clear( USES_SHIELD);
	}
	if (pl.used.get( USES_EMERGENCY_SHIELD)) {
	    pl.used.clear( USES_EMERGENCY_SHIELD);
...
{
    if (on)
	pl.obj_status.get( THRUSTING);
    else
	pl.obj_status.clear( THRUSTING);
}
...
static void Misc_object_update(void)
{
	...
	if (obj.obj_status.get( WARPING))
	    Object_warp(obj);

	if (obj.obj_status.get( WARPED))
...
static void Update_visibility(player_t *pl, int ind)
{
	...
	if (ind == j || !Player_is_cloaked(pl_j))
	    pl.visibility[j].canSee = true;
		 ...
		 || (int)(Math.random() * UPDATE_RATE)
		 < Math.abs(frame_loops - pl.visibility[j].lastChange)) {

	    pl.visibility[j].lastChange = frame_loops;

	    if ((Math.random() * (pl.item[ITEM_SENSOR] + 1))
		> (Math.random() * (pl_j.item[ITEM_CLOAK] + 1)))
		pl.visibility[j].canSee = true;
	    else
...
static void Update_players(void)
{
	 ...
	 */
	if (pl.obj_status.get( WARPING))
	    Player_warp(pl);

	/*
	 * Reset WARPED status, when player is outside a wormhole
	 */
	if (pl.obj_status.get( WARPED))
	...
	if (options.legacyMode) {
	    ...
	    if (pl.obj_status.get( GRAVITY)) {
		Point2D gravity = World_gravity(pl.pos);
...
void Update_objects(void)
{
	...
	if (pl.stunned > 0) {
		...
		pl.stunned = 0;
	    pl.used.clear( USES_SHIELD);
	    pl.used.clear( USES_LASER);
	    pl.used.clear( USES_SHOT);
	    pl.did_shoot = false;
	    Thrust(pl, false);
	}
	if (pl.used.get( USES_SHOT) || pl.did_shoot)
	    Fire_normal_shots(pl);
	if (pl.used.get( USES_LASER)) {
