 ...
 */
typedef struct {
    Click 	unrealWorld;		/* Lower left hand corner is this */
					/* world coordinate */
    Click 	realWorld;		/* If the player is on the edge of
 ...
 * in view.
 */
static inline bool clpos_inview(click_visibility_t *v, Click  pos)
{
    Click  wpos = v.unrealWorld, rwpos = v.realWorld;
...
static void fastshot_end(connection_t *conn)
{
	...
	if (fastshot_num[i] != 0) {
	    Send_fastshot(conn, i,
			  (unsigned String ) fastshot_ptr[i],
			  fastshot_num[i]);
	    fastshot_num[i] = 0;
	}
    }
...
static void Frame_radar_buffer_add(Click  pos, int s)
{
    radar_t *p;
...
static void Frame_radar_buffer_send(connection_t *conn, player_t *pl)
{
    ...
    if (num_radar > Math.min(256, MAX_SHUFFLE_INDEX))
	num_radar = Math.min(256, MAX_SHUFFLE_INDEX);
	...
	/* permute. */
	for (i = 0; i < num_radar; i++) {
	    dest = (int)(Math.random() * (num_radar - i)) + i;
	    tmp = radar_shuffle[i];
...
static int Frame_status(connection_t *conn, player_t *pl)
{
    ...
    pl.lock.tagged.clear( LOCK_VISIBLE);
	...
	if ((!world.rules.mode.get( LIMITED_VISIBILITY)
...
static void Frame_map(connection_t *conn, player_t *pl)
{
    ...
    packet_count = 0;
    max_packet = Math.max(5, bytes_left / target_packet_size);
    i = Math.max(0, pl.last_target_update);
	...
	if (targ.update_mask.get( conn_bit)
	    || (targ.conn_mask.get( conn_bit) == 0
    ...
    packet_count = 0;
    max_packet = Math.max(5, bytes_left / cannon_packet_size);
    i = Math.max(0, pl.last_cannon_update);
	...
	if (clpos_inview(&cv, cannon.pos)) {
	    if (cannon.conn_mask.get( conn_bit) == 0) {
    ...
    packet_count = 0;
    max_packet = Math.max(5, bytes_left / fuel_packet_size);
    i = Math.max(0, pl.last_fuel_update);
	...
	fs = Fuel_by_index(i);
	if (fs.conn_mask.get( conn_bit) == 0) {
    ...
    packet_count = 0;
    max_packet = Math.max(5, bytes_left / polystyle_packet_size);
    i = Math.max(0, pl.last_polystyle_update);
	...
	poly = &pdata[i];
	if (poly.update_mask.get( conn_bit)) {
...
static void Frame_shuffle_objects()
{
    ...
    num_object_shuffle = Math.min(NumObjs, options.maxVisibleObject);
...
static void Frame_shuffle_players()
{
    ...
    num_player_shuffle = Math.min(NumPlayers, MAX_SHUFFLE_INDEX);
    ...
    /* permute. */
    for (i = 0; i < num_player_shuffle; i++) {
	int j = (int)(Math.random() * (num_player_shuffle - i) + i);
	...
	case OBJ_MINE:
		...
		if (mine.id != NO_ID
		    ...
		    laid_by_team = (Team_immune(mine.id, pl.id)
				    || (mine.obj_status.get( OWNERIMMUNE)
					&& mine.mine_owner == pl.id));
		    if (confused) {
			id = 0;
			laid_by_team = (Math.random() < 0.5);
		    }
		}
...
static void Frame_ships(connection_t *conn, player_t *pl)
{
	...
	Click 	pos = (tpl ? tpl.pos : trans.pos);
	...
	if (pl.visibility[i].canSee
	    ...
	    Send_ship(conn,
		      ...
		      pl_i.id,
		      pl_i.dir,
		      pl_i.used.get( HAS_SHIELD) != 0,
		      Player_is_cloaked(pl_i) ? 1 : 0,
		      pl_i.used.get( HAS_EMERGENCY_SHIELD) != 0,
		      Player_is_phasing(pl_i) ? 1 : 0,
		      pl_i.used.get( USES_DEFLECTOR) != 0
	    );
	}
...
static void Frame_radar(connection_t *conn, player_t *pl)
{
	...
	for (i = 0; i < NumObjs; i++) {
	    shot = Obj[i];
	    if (!OBJ_TYPEBIT(shot.type).get( mask))
		continue;
...
void Frame_update()
{
	...
	player_fps = Math.min(player_fps, pl.player_fps);
...
void Set_player_message_f(player_t *pl, String fmt, ...)
{
    size_t len;
    static char msg[2 * MSG_LEN];
    va_list ap;
