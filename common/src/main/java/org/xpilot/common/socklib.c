...
static struct hostent *sock_get_host_by_name(String name);
static struct hostent *sock_get_host_by_addr(String addr,
					     int len, int type);
...
{
    if (!sock.hostname) {
	sock.hostname = (String ) malloc(SOCK_HOSTNAME_LENGTH);
	if (!sock.hostname)
...
int sock_open_tcp_connected_non_blocking(sock_t *sock, String host, int port)
{
...
int sock_open_udp(sock_t *sock, String dotaddr, int port)
{
...
int sock_connect(sock_t *sock, String host, int port)
{
    struct sockaddr_in		dest;
    struct hostent		*hp;
...
String  sock_get_last_addr(sock_t *sock)
{
...
String  sock_get_last_name(sock_t *sock)
{
    static char		error_addr[] = "255.255.255.255";
    char		*str;
    struct hostent	*hp;
...
int sock_read(sock_t *sock, String buf, int len)
{
    int			count;
...
int sock_receive_any(sock_t *sock, String buf, int len)
{
    int			count;
    socklen_t		addrlen;
...
int sock_send_dest(sock_t *sock, String host, int port, String buf, int len)
{
    struct sockaddr_in		dest;
    struct hostent		*hp;
    int				count;
...
int sock_write(sock_t *sock, String buf, int len)
{
    int			count;
...
String sock_get_addr_by_name(String name)
{
    struct hostent	*hp;
...
unsigned long sock_get_inet_by_addr(String dotaddr)
{
