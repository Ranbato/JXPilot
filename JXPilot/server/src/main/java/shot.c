			...
			Click  pos, Point2D vel, modifiers_t mods)
{
	...
	mass = (status.get( FROMCANNON) ? MINE_MASS * 0.6 : MINE_MASS);
	used = 1;
    }
...
void Update_connector_force(ballobject_t *ball)
{
    ...
    if (Math.abs(ratio) > options.maxBallConnectorRatio) {
	Detach_ball(pl, ball);
	return;
    }
