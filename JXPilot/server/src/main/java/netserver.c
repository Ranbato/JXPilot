...
static void Feature_init(connection_t *connp)
{
    ...
    if (v < 0x4F00) {
	if (v >= 0x4210)
	    features.get( F_TEAMRADAR);
	if (v >= 0x4300)
	    features.get( F_SEPARATEPHASING);
	if (v >= 0x4400)
	    features.get( F_ASTEROID);
	if (v >= 0x4401)
	    features.get( F_FASTRADAR);
	if (v >= 0x4500)
	    features.get( F_FLOATSCORE);
	if (v >= 0x4501)
	    features.get( F_TEMPWORM);
    }
    else {
	features.get( F_POLY);
	features.get( F_TEAMRADAR);
	...
	if (v >= 0x4F12) {
	    features.get( F_SHOW_APPEARING);
	    features.get( F_SENDTEAM);
	}
	if (v >= 0x4F13)
	    features.get( F_CUMULATIVETURN);
	if (v >= 0x4F14)
	    features.get( F_BALLSTYLE);
	if (v >= 0x4F15)
