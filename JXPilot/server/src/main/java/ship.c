...
void Make_thrust_sparks(player_t *pl)
{
    ...
    Click  engine = Ship_get_engine_clpos(pl.ship, pl.dir), pos;
    int afterburners;
    ...
    /* floor(tot_sparks + Math.random()) randomly rounds up or down to an integer,
    ...
    Make_debris(pos,
		...
		RED,
		8,
		(int)((tot_sparks-alt_sparks) + Math.random()),
		min_dir, max_dir,
		1.0, max_speed,
		3.0, max_life);
...
void Delta_mv_elastic(object_t *obj1, object_t *obj2)
{
	...
	&& obj2.id != NO_ID
	&& obj2.obj_status.get( COLLISIONSHOVE)) {
		 ...
		 double   min_life,  double  max_life)
{
	...
	dir = MOD2(min_dir + (int)(Math.random() * (max_dir - min_dir)), RES);
		   ...
		   double   min_life,     double max_life)
{
	...
	/* Direction */
	dir = MOD2(min_dir + (int)(Math.random() * MOD2(max_dir - min_dir, RES)),
		   RES);
	...
	wreckage.wire_turnspeed = 0.02 + Math.random() * 0.35;
	wreckage.wire_rotation = (int)(Math.random() * RES);
	size = (int) ( 256.0 * 1.5 * mass / total_mass );
	if (size > 255)
	    size = 255;
	wreckage.wire_size = size;
	wreckage.wire_type = (uint8_t)(Math.random() * 256);
...
void Explode_fighter(player_t *pl)
{
    ...
    Make_debris(pl.pos,
		...
		RED,
		8,
		(int)(min_debris + debris_range * Math.random()),
		0, RES-1,
    ...
    Make_wreckage(pl.pos,
		  ...
		  pl.id,
		  pl.team,
		  Math.max(pl.mass/8.0, 0.33), pl.mass,
		  2.0 * pl.mass,
		  GRAVITY,
		  10,
		  0, RES-1,
