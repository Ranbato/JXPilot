...
#define BITMAP_FUEL_BORDER 3
	...
	image = Math.abs((loopsSlow + x + x * y) % (fuel_images * 2));
...
void Gui_paint_polygon(int i, int xoff, int yoff)
{
    ...
    if (style.flags.get( STYLE_INVISIBLE)) return;
    ...
    if ((filled || textured) && (BIT(style.flags,
        ...
        if (textured && style.flags.get( STYLE_TEXTURED)) {
