...
ConsoleInformation *CON_Init(String FontName,
			     ...
			     SDL_Rect rect)
{
    ...
    newinfo.Prompt = (String )&CON_DEFAULT_PROMPT;
	...
	(char **) malloc(sizeof(String ) * newinfo.LineBuffer);
    newinfo.CommandLines =
	(char **) malloc(sizeof(String ) * newinfo.LineBuffer);
    for (loop = 0; loop <= newinfo.LineBuffer - 1; loop++) {
	newinfo.ConsoleLines[loop] =
	    (String ) calloc(CON_CHARS_PER_LINE + 1, sizeof(char));
...
void CON_Out(ConsoleInformation * console, String str, ...)
{
    va_list marker;
...
int CON_Background(ConsoleInformation * console, String image, int x,
		   int y)
{
    SDL_Surface *temp;
...
void CON_SetPrompt(ConsoleInformation * console, String newprompt)
{
    if (!console)
	return;
...
void CON_Execute(ConsoleInformation * console, String command)
{
    if (console)
...
void CON_SetExecuteFunction(ConsoleInformation * console,
			    void (*CmdFunction) (ConsoleInformation *
						 console2, String command))
{
    if (console)
	console.CmdFunction = CmdFunction;
}

void Default_CmdFunction(ConsoleInformation * console, String command)
{
...
void CON_SetTabCompletion(ConsoleInformation * console,
			  String (*TabFunction) (String command))
{
    if (console)
...
String Default_TabFunction(String command)
{
...
void Add_String_to_Console(String text)
{
