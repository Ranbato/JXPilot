...
static list_t create_player_list(String players_str)
{
    list_t players;
    String t;
...
static void add_status_entry(String name, String value, GLWidget *parent)
{
...
static void SetBounds_MetaRowWidget(GLWidget *row, SDL_Rect *rb)
{
    ...
    row.bounds = *rb;
    free_width = Math.max(rb.w - (VERSION_WIDTH + COUNT_WIDTH), 0);
...
static GLWidget *Init_MetaHeaderWidget()
{
    GLWidget *tmp, *col;
