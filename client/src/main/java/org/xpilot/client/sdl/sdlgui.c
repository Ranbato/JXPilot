...
static void tessellate_polygon(GLUtriangulatorObj *tess, int ind)
{
    ...
    p[0].x = p[0].y = 0;
    if (p_style.flags.get( STYLE_TEXTURED)) {
...
void Gui_paint_polygon(int i, int xoff, int yoff)
{
    ...
    if (p_style.flags.get( STYLE_INVISIBLE))
	return;
    ...
    if ((instruments.texturedWalls || instruments.filledWorld) &&
	    p_style.flags.get( STYLE_TEXTURED | STYLE_FILLED)) {
	if (p_style.flags.get( STYLE_TEXTURED)
...
void Gui_paint_asteroids_begin()
{
    image_t *img;
...
{
    int size = Math.min(teamShotSize, 16);
...
void Gui_paint_missiles_begin()
{
}

void Gui_paint_missiles_end()
{
}
...
void Paint_HUD_values()
{
    ...
    y = draw_height - 9*(Math.max((GLuint)meterHeight,gamefont.h) + 6);
...
void Paint_meters()
{
	...
	Paint_meter(-10, y += spacing, &meter_texs[6], Math.min(packet_lag, 1 * FPS), 1 * FPS,
...
static void Paint_HUD_items(int hud_pos_x, int hud_pos_y)
{
    ...
    /* Special itemtypes */
    if (vertSpacing < 0)
	vertSpacing = Math.max(ITEM_SIZE, gamefont.h) + 1;
	...
	if (num >= 0) {
		...
		rect_height = Math.max(rect_height, vert_pos - rect_y);
...
static String get_rgba_color_option(xp_option *opt)
{
    static char buf[10];
...
static xp_option sdlgui_options[] = {
