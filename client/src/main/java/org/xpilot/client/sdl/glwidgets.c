...
static void option_callback( void *opt, String value );
    ...
    if (over) {
	    ...
	    for ( end=0 ; end < 1000000; ++end) {
		    ...
		    continue;
    	    	...
    	    	if (start != end ) {
		    strncpy(&line[0],help+start,Math.min(255,end-start));
...
static void option_callback( void *tmp, String value )
{
    xp_option *opt;
...
void DrawGLWidgetsi( GLWidget *list, int x, int y, int w, int h)
{
    ...
    while (curr) {
    	x2 = Math.max(x,curr.bounds.x);
    	y2 = Math.max(y,curr.bounds.y);
    	w2 = Math.min(x+w,curr.bounds.x+curr.bounds.w) - x2;
    	h2 = Math.min(y+h,curr.bounds.y+curr.bounds.h) - y2;
	if ( (w2 > 0) && (h2 > 0) ) {
    	    glEnable(GL_BLEND);
...
static void motion_ScrollbarWidget( Sint16 xrel, Sint16 yrel, Uint16 x, Uint16 y, void *data )
{
    ...
    if (wid_info.oldmoves > 0) {
    	coord2 = Math.min(max-size,*coord1+wid_info.oldmoves);
    } else if (wid_info.oldmoves < 0) {
    	coord2 = Math.max(min,*coord1+wid_info.oldmoves);
    }
...
void ScrollbarWidget_SetSlideSize( GLWidget *widget, GLfloat size )
{
    ...
    sb.size = Math.min(1.0f,Math.max(0.0f,size));
				...
				void *poschangedata )
{
    ...
    wid_info.pos   	= Math.max(0.0f,Math.min(1.0f,pos));
    wid_info.size  	= Math.max(0.0f,Math.min(1.0f,size));
    wid_info.dir   	= dir;
    wid_info.oldmoves	= 0;
...
static void Paint_LabelWidget( GLWidget *widget )
{
    ...
    if (scraptarget == tmp) {
	alpha = Math.max(0,Math.min(255,(color & 255) + tsin(flasher)*64));
					...
					void *actiondata, bool start_state )
{
    ...
    tmp.bounds.w   	= Math.max(ontex.width,offtex.width)+5;
    tmp.bounds.h   	= Math.max(ontex.height,offtex.height);
    	    	    	    	 ...
    	    	    	    	 void (*callback)(void *tmp, String value), void *data )
{
    ...
    tmp.bounds.h   	= 1 + Math.max( wid_info.name.bounds.h,wid_info.buttonwidget.bounds.h) + 1 ;
...
static void IntChooserWidget_Add( void *data )
{
    ...
    if (tmp.direction > 0) {
     	step = (++tmp.duration)*(tmp.maxval-tmp.minval)/(Math.max(1,Math.min(maxFPS,FPS))*3);
    } else {
    	step = 1;
    }
    ...
    if (*(tmp.value) > tmp.maxval) {
    	...
    	tmp.duration = 0;
    	*(tmp.value) = Math.min( *(tmp.value) + step, tmp.maxval);
...
static void IntChooserWidget_Subtract( void *data )
{
    ...
    if (*(tmp.value) < tmp.minval) {
    	...
    	tmp.duration = 0;
    	*(tmp.value) = Math.max( (*(tmp.value)) - step, tmp.minval);
...
static void Callback_ColorModWidget(void *tmp, String value);
    	    	    	    	    ...
    	    	    	    	    void (*callback)(void *tmp, String value), void *data )
{
    ...
    maxwidth = Math.max(maxwidth,wid_info.redpick.bounds.w);
    maxwidth = Math.max(maxwidth,wid_info.greenpick.bounds.w);
    maxwidth = Math.max(maxwidth,wid_info.bluepick.bounds.w);
    maxwidth = Math.max(maxwidth,wid_info.alphapick.bounds.w);
...
static void SetBounds_ListWidget( GLWidget *widget, SDL_Rect *b )
{
    ...
    while(curr) {
    	if (tmp.direction == VERTICAL) {
    	    bounds.w = Math.max(bounds.w,curr.bounds.w);
	    bounds.h += curr.bounds.h;
	} else {
    	    bounds.w += curr.bounds.w;
	    bounds.h = Math.max(bounds.h,curr.bounds.h);
	}
	curr = curr.next;
    }
...
static void SetBounds_ScrollPaneWidget(GLWidget *widget, SDL_Rect *b )
{
	...
	if (wid_info.hori_scroller) {
	    ScrollbarWidget_SetSlideSize(wid_info.hori_scroller,Math.min(((GLfloat)wid_info.masque.bounds.w)/((GLfloat)bounds.w),1.0f));
    	    pos = Math.min( ((ScrollbarWidget *)(wid_info.hori_scroller.wid_info)).pos, 1.0f - ((ScrollbarWidget *)(wid_info.hori_scroller.wid_info)).size);
	...
	if (wid_info.vert_scroller) {
	    ScrollbarWidget_SetSlideSize(wid_info.vert_scroller,Math.min(((GLfloat)wid_info.masque.bounds.h)/((GLfloat)bounds.h),1.0f));
    	    pos = Math.min( ((ScrollbarWidget *)(wid_info.vert_scroller.wid_info)).pos, 1.0f - ((ScrollbarWidget *)(wid_info.vert_scroller.wid_info)).size);
...
static void SetBounds_MainWidget( GLWidget *widget, SDL_Rect *b )
{
    ...
    for ( i=0; i < 5 ; ++i) {
	    ...
	    if ( Math.abs(bs.x - bw.x) > ABS( diff = (bw.x+bw.w) - (bs.x+bs.w) ) ) {
	    	bs.x = b.x + b.w - diff - bs.w;
		change = true;
	    }

	    if ( Math.abs(bs.y - bw.y) > ABS( diff = (bw.y+bw.h) - (bs.y+bs.h) ) ) {
...
extern int Console_isVisible();
extern void Paste_String_to_Console(String text);
static void button_MainWidget( Uint8 button, Uint8 state , Uint16 x , Uint16 y, void *data )
{
    int scraplen;
