...
static void Compute_bounds_radar(Point  *min, Point  *max, const Rectangle *b)
{
...
static int Radar_init(GLWidget *widget)
{
    ...
    if (!radar_surface) {
        logger.error("Could not create radar surface: {}", SDL_GetError());
        return -1;
    }
 ...
 */
GLWidget *Init_RadarWidget()
{
    GLWidget *tmp	= Init_EmptyBaseGLWidget();
    if ( !tmp ) {
        logger.error("Failed to malloc in Init_RadarWidget");
	return null;
    }
...
static xp_option radar_options[] = {
    XP_STRING_OPTION(
	"radarGeometry",
	"200x200+10+10",
	null,
