...
{
    xp_option *opt = Find_option("keyExit");
    char msg[MSG_LEN];
    String val;
...
static inline int pointer_button_index_by_option(xp_option *opt)
{
...
bool Key_release(keys_t key)
{
    ...
    case KEY_DROP_MINE:
    case KEY_DETACH_MINE:
	if (auto_shield && shields && !keyv.get( KEY_SHIELD)) {
	    /* Here We need to know if any other weapons are still on */
	    /*      before we turn shield back on   */
	    keyv.clear( key);
	    if (!keyv.get( KEY_FIRE_SHOT) &&
		!keyv.get( KEY_FIRE_LASER) &&
		!keyv.get( KEY_FIRE_MISSILE) &&
		!keyv.get( KEY_FIRE_TORPEDO) &&
		!keyv.get( KEY_FIRE_HEAT) &&
		!keyv.get( KEY_DROP_MINE) &&
		!keyv.get( KEY_DETACH_MINE))
		keyv.set( KEY_SHIELD);
	}
	break;
...
void Set_toggle_shield(bool on)
{
	...
	if (auto_shield)
	    ...
	    shields = (keyv.get( KEY_SHIELD)) ? true : false;
    }
}
...
static bool setPointerButtonBinding(xp_option *opt, String value)
{
	...
	for (j = 0; j < num_options; j++) {
	    xp_option *opt_j = Option_by_index(j);
	    String opt_j_name;
	    keys_t opt_j_key;
...
static String getPointerButtonBinding(xp_option *opt)
{
    int ind = pointer_button_index_by_option(opt);
