...
typedef struct widget_string {
    String str;		/* Current input string */
} widget_string_t;
...
static int Widget_create(widget_type_t type, String name, Window window,
...
static void Widget_draw_button(widget_t *widget, bool inverse, String label)
{
    int			x, y;
    unsigned long	fg, bg;
...
static void Widget_draw_input(widget_t *widget, String str)
{
...
static void Widget_draw_color(widget_t *widget, String str, int color)
{
...
static void Widget_draw_viewer(widget_t *widget, XExposeEvent *expose)
{
    ...
    if (expose != null) {
	y_0 = Math.max(y_0, expose.y);
	y_1 = Math.min(y_1, expose.y + expose.height);
    }
    else
	y_1 = Math.min(y_1, viewerw.visible_y + widget.height);
...
static Bool Widget_check_motion(Display *d, XEvent *e, String p)
{
...
static void Widget_button_motion(XEvent *event, int widget_desc)
{
	...
	cm.found = 0;
	XCheckIfEvent(dpy, &dumb, Widget_check_motion, (String )&cm);
	if (cm.found > 0)
	    break;
...
int Widget_update_viewer(int popup_desc, String buf, int len)
{
    ...
    last_visible = (viewer_sub.visible_y + viewer_widget.height - 20)
		    / text_height;
    start = Math.max(0, viewer_sub.num_lines - 1);
    Widget_viewer_calculate_text(viewer_desc);
    end = viewer_sub.num_lines;
    start = Math.max(first_visible, start);
    end = Math.min(last_visible + 1, end);
    if (start < end)
