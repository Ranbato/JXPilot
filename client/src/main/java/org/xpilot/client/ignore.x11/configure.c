...
static int Config_creator(xp_option *opt, int widget_desc, int *height);
...
static int Nelem_config_creator()
{
...
static xp_option *Config_creator_option(int i)
{
    int ind = -1;
...
static void Create_config()
{
	...
	xp_option *opt = Config_creator_option(i);
...
void Config_init()
{
    ...
    max_ids = Math.max(num_color_options, num_default_options) + 1;
