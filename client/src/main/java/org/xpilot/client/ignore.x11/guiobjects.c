...
Point2D *asteroidShapes[NUM_ASTEROID_SHAPES][NUM_ASTEROID_POINTS];
...
int Init_asteroids()
{
	...
	for ( i = 0; i < NUM_ASTEROID_POINTS; i++ ) {
	    asteroidShapes[shp][i] = (Point2D *) dynmem;
	    dynmem += point_size;
...
void Gui_paint_fastshot(int color, int x, int y)
{
	...
	int s_size = Math.min(shotSize, 16);
	int z = s_size / 2;
...
void Gui_paint_teamshot(int x, int y)
{
	...
	int s_size = Math.min(teamShotSize, 16);
	int z = s_size / 2;
...
static xp_option guiobject_options[] = {
    COLOR_INDEX_OPTION(
	"teamShotColor",
	2,
	&teamShotColor,
