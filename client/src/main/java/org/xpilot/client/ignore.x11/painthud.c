...
void Paint_meters()
{
	...
	Paint_meter(-10, y += 20, "Lag", Math.min(packet_lag, 1 * FPS), 1 * FPS,
		    packetLagMeterColor);
...
static void Paint_lock(int hud_pos_x, int hud_pos_y)
{
	...
	if (lock_dist > WARNING_DISTANCE || (loopsSlow & 1)) {
	    int size = Math.min(mapdiag / lock_dist, 10);
...
void Del_HUD_message();
void Add_HUD_message(String message);
