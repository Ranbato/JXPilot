...
static void Talk_create_window()
{
    /*
     * Create talk window.
     */
 ...
 */
static void Add_msg_to_history(String message)
{
    String tmp;
    char **msg_set;
    int i;
 ...
 *
 */
static String Get_msg_from_history(int* pos, String message, keys_t direction)
{
    int i;
    char **msg_set;
 ...
 */
static void Talk_delete_emphasized_text()
{
...
int Talk_do_event(XEvent *event)
{
	...
	if (XLookupString(&event.xkey, &ch, 1, &keysym, &compose)
	    ...
	    String tmp;		/* for receiving a line from the history */
...
static int Text_width_to_pos(XFontStruct *font, String text, size_t len,
			     int width)
{
...
static void Clear_talk_selection()
{
...
static void Clear_draw_selection()
{
 ...
 */
static void Selection_set_state()
{
    if (selection.len > 0)
