...
public class Xpm
{
	...
	private static final Logger logger = LoggerFactory.getLogger(Xpm.class);
	

	// todo Map.of when Java 9 is common
	private static final int rgbents[][] = {
{255, 250, 250},
{248, 248, 255},
{248, 248, 255},
{245, 245, 245},
...
{144, 238, 144}
	...
	private static final String rgbnames[] = {
"snow",
"ghost white",
"GhostWhite",
"white smoke",
...
"light"
	...
	private static final HashMap<String,int[]> colorMap = new HashMap<>(rgbnames.length);

    /**
     * Initialize a hashmap with the lowercase version of the strings and the RGB triplets
     */
	static{
        for (int i = 0;i<rgbnames.length;i++) {
            colorMap.put(rgbnames[i].toLowerCase(),rgbents[i]);
        }
    }
    /**
     *     look up RGB triplet by string name of color.
     *     @return int[3] of rgb, or null if color not found
     */
	...
	public static int NameToRGB(String str)
	{
        ...
        int [] rgb = colorMap.get(str.toLowerCase());
		if(rgb != null){
				int ret;
				ret =   0xff000000 |
					(rgb[0]<<16) |
		...
		if(!str.equalsIgnoreCase("None")) {
            logger.debug("NameToRGB: Could not find match for color " + str);
        }
		return 0x00000000;
	}
	...
	public static String rgb3ToName(int val[])
	{
		...
		{
            if (val[0] == rgbents[count][0] && val[1] == rgbents[count][1] && val[2] == rgbents[count][2]) {
	...
	public static Image xpmToImage(String xpm) {
		...
		*/
		...
		if (xpm == null) {
			logger.debug("XpmToImage : Provided xpm is null!");
			return null;
		}
		...
		{
			logger.debug("xpm data doesn't start with XPM magic but with '{}'. exit",xpm.substring(0,10));
			return null;
		}
		...
		logger.debug("width={},height={},colcount={},cpp={}",width,height,colcount,charsperpixel);
		...
		{
			if(parse==0){
				logger.debug("ERROR: expecting color def");
				return null;
			}
			...
			String colorname =
				...
				logger.debug("colorname on line is {}",colorname);
				logger.debug("now parsing {}",xpm.substring(parse,parse+charsperpixel));
			...
			if(charsperpixel==1){
				...
				logger.debug("two charsperpixel: substre=={} which generates char {}",
			...
			logger.debug("Color num parsed for \"{}\"({}) is #{}",colorname,
		...
		logger.debug("Done with color defs");
		...
		if(parse==-1)
		{
			logger.debug("ERROR; incomplete Xpm data");
			return null;
		}
		parse+=1;

		logger.debug("Xpm starting image parse");
		while(xpm.charAt(parse) =='"')
		{
			parse++;
			parseend = xpm.indexOf('"', parse);
			
			logger.debug("Xpm; parsing \""+xpm.substring(parse, parseend)+"\"");
			...
			for(int pix=parse; pix<parseend;pix++)
			{
				...
				if(pixval==null){
					logger.debug("HEY MORON: no value stored for int {}, char substring ",
		...
		{
			logger.debug("Warning.. pixmap truncated!");
		}
	...
	{
		//logger.debug("getColorName passed: "+xpmline);
		...
		if(parsemid <=0)
		{
			logger.debug("Xpm: oops. no c found, in: {}",xpmline);
			return null;
		}
		parsemid+=3;
