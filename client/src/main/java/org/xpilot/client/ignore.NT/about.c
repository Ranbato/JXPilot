...
static void About_create_window()
{
...
int Handle_motd(long off, String buf, int len, long filesize)
{
    ...
    if (!motd_buf) {
	motd_size = Math.min(filesize, MAX_MOTD_SIZE);
	i = Math.max(motd_size, (long)(sizeof no_motd_msg)) + 1;
...
#ifdef _WINDOWS
int Credits_callback(int widget_desc, void *data, String *unused)
{
